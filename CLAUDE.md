<<<<<<< HEAD
# Coachify Account API — Service Documentation

## Developer Notes

### What this service does
The **authentication and identity hub** for the entire platform. Every user (coach or client) is created, verified, and logged in through this service. It issues JWT tokens that all other services validate.

### Things that have caused bugs before
- **OAuth state cookies expire after 600s** — if a user takes too long on the Google/Facebook consent screen, the callback fails with an opaque error. The state is stored in a secure cookie, not a session.
- **ExternalID vs MongoDB _id** — Users have BOTH. `_id` is the Mongo ObjectID, `ExternalID` is generated by the identifiers-api (e.g., `USR-abc123`). Other services reference users by `ExternalID`. Never confuse them.
- **Password hashing is bcrypt** — The `crypto.go` utility handles this. Never store or compare passwords as plain text. Use timing-safe comparison.
- **Confirmation codes have no enforced TTL in the DB** — expiry is checked in application logic, not via MongoDB TTL index. If you add a new code-based flow, enforce expiry in the service layer.

### How to think about changes here
- Auth flows are sequential: register → confirm email → login → get JWT.
- OAuth flows skip email confirmation and go straight to JWT issuance.
- The service calls 4 other services during registration (identifiers, notification, invitation, payments). Any of these failing will break signup. The calls are synchronous — no retries.
- Coach-client relationships live in a separate `coach_clients` collection, managed by `CoachService`.

---
=======
# Coachify Account API - Service Architecture Guide
>>>>>>> 1ac64bd0b63f018e0d5544c7fd1b6f50b50d4748

## 1. WHAT THIS SERVICE DOES

### Primary Responsibility
<<<<<<< HEAD
User authentication, profile CRUD, OAuth2 integration (Google, Facebook), JWT token issuance/refresh, email verification flows, password reset, and coach-client relationship management.

### Data Owned (MongoDB database: `users`)
| Collection | Purpose |
|---|---|
| `users` | All user profiles (coaches and clients) |
| `coach_clients` | Coach-to-client relationship mapping |

### External Dependencies
| Service | Method | Purpose |
|---|---|---|
| coachify-identifiers-api | `GET /identifier/{label}` | Generate ExternalIDs on registration |
| coachify-notification-api | `POST /emails` | Send confirmation/reset emails via Mailgun |
| coachify-invitation-api | `POST /invitation/accept` | Accept pending invitation during signup |
| coachify-payments-api | `POST /subscription/trial` | Enroll new user in trial plan |
| Google OAuth2 | OAuth2 flow | Social login |
| Facebook OAuth2 | OAuth2 flow | Social login |
=======
...
|---|---|---|---|
| GET | `/` | GetHealth | Health check |
| GET | `/health` | GetHealth | Health check |
| GET | `/metrics` | Prometheus | Prometheus metrics |
| GET | `/oauth/:provider/login` | OAuth2Login | Start OAuth2 flow (google/facebook) |
| POST | `/oauth/:provider/auth/me` | OAuth2ServerSideCallback | Server-side OAuth callback |
| POST | `/user/signup` | Register | Create new account |
| POST | `/user/confirm` | Confirm | Verify email with code |
| POST | `/user/resend-confirm` | ResendConfirmEmail | Resend verification email |
| POST | `/user/login` | Login | Authenticate and get JWT |
| POST | `/user/reset-password/init` | InitResetPassword | Request password reset email |
| POST | `/user/reset-password/confirm` | ConfirmResetPassword | Set new password with code |
| POST | `/user/refresh-token` | RefreshToken | Refresh expired JWT |
| GET | `/user/` | GetAllUsersPag | List users (paginated) |
| DELETE | `/user/` | DeleteUser | Delete user |
| POST | `/user/` | AddUser | Admin: create user directly |
| GET | `/user/get-user-by-email/:prefix` | GetUserByEmail | Lookup by email |
| GET | `/user/check-email/:prefix` | GetUserByEmail | Check if email exists |
| GET | `/user/get-user-by-id/:prefix` | GetUserById | Lookup by MongoDB ID |
| GET | `/user/get-user/:prefix` | GetUserByExternalId | Lookup by ExternalID |

### Protected Routes (JWT Required)
| Method | Path | Handler | Purpose |
|---|---|---|---|
| PUT | `/user/update-user` | UpdateUser | Update own profile |
| GET | `/coach/clients` | ListCoachClients | List coach's assigned clients |
| GET | `/coach/client` | GetCoachIDByClientID | Get coach for a given client |
| DELETE | `/coach/client/:client_id` | DissociateCoachClient | Remove coach-client link |
...
User {
    ID, ExternalID, UserFirstname, UserLastname, UserEmail, UserPassword,
    UserRole, UserProfilePicture, UserDescription, UserPhoneNumber,
    Token, RefreshToken, Providers (OAuthUser slice),
    VerificationStatus, Gender, Status, ConfirmCode, ResetPasswordCode,
    Address, Metadata, CreatedAt, UpdatedAt, LastLogin
}

// CoachClient — Relationship mapping
CoachClient {
    ID, CoachExternalID, ClientExternalID, CreatedAt, UpdatedAt, Status
}
```

**Password Rule**: Minimum 8 chars, must contain uppercase, lowercase, digit, and symbol.
...
```
├── main.go           # Entry point
├── app/app.go        # App lifecycle (init DB, services, router, server)
├── router/
│   ├── router.go     # All route registration
│   └── middlewares.go # JWT auth middleware, security headers, CORS
├── handlers/
│   ├── auth.go       # Registration, login, OAuth2, password reset, token refresh
│   ├── coach.go      # Coach-client relationship endpoints
│   ├── health.go     # Health check
│   └── missing.go    # 404 handler
├── services/
│   ├── auth.go       # Core auth business logic (register, login, OAuth, confirm, reset)
│   ├── coach.go      # Coach-client management logic
│   └── services.go   # Service container and DI wiring
├── repositories/
│   ├── User.go       # User CRUD operations
│   ├── Coach.go      # Coach-client queries
│   └── indexes.go    # MongoDB index initialization
├── models/
│   ├── db/           # User, CoachClient, OAuthUser, Address, constants
│   ├── api/          # Request/Response DTOs
│   ├── mapping/      # DB ↔ API struct mappers
│   ├── masks/        # Field masks for partial updates
│   └── apiError.go   # Shared error definitions
├── pkg/
│   ├── identifier/   # Client for identifiers-api
│   ├── notification/  # Client for notification-api
│   ├── invitation/    # Client for invitation-api
│   └── payments/      # Client for payments-api
├── oauth2/            # OAuth2 provider setup (Google, Facebook)
├── core/              # Core utilities
└── utils/
    ├── config.go      # Viper env loading (singleton)
    ├── logger.go      # Zap logger
    ├── crypto.go      # Bcrypt hashing, AES encryption
...
| `BASE_URL` | http://localhost:8086 | Self URL |
| `MONGODB_URI` | (atlas connection) | MongoDB connection string |
| `IDENTIFIER_API_URL` | (heroku URL) | Identifiers service |
| `NOTIFICATION_API_URL` | (heroku URL) | Notification service |
| `INVITATION_API_URL` | (heroku URL) | Invitation service |
| `PAYMENT_API_URL` | http://localhost:8088 | Payments service |
| `CHAT_API_URL` | http://localhost:8088 | Chat service |
| `PLAN_HEX_COACH` | (hex ID) | Default coach plan ID |
| `PLAN_HEX_CLIENT` | (hex ID) | Default client plan ID |
| `COACHIFY_ENCRYPTION_KEY` | (32-char key) | AES-GCM token encryption |
| `COACHIFY_SECRET_KEY` | (hex key) | HMAC JWT signing |
| `GOOGLE_APP_ID` / `GOOGLE_APP_SECRET` | — | Google OAuth2 credentials |
| `FACEBOOK_APP_ID` / `FACEBOOK_APP_SECRET` | — | Facebook OAuth2 credentials |
| `MAILGUN_API_KEY` / `MAILGUN_DOMAIN` | — | Email sending config |
| `CONFIRMATION_CODE_TEMPLATE_PENDING` | confirmation_code_pending | Mailgun template name |
| `WELCOME_TEMPLATE` | welcome_template | Mailgun welcome template |
=======
